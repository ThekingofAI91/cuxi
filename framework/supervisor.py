...
图结构 = supervisor → retriever/analyzer/verifier/summarizer
...
重要：请始终以角色设定的身份、语气和风格回答，用第一人称，保持角色一致性。"""
...
async def supervisor_node(state: AgentState) -> dict[str, Any]:
    ...
    1. 历史轮次超阈值 → summarizer
    2. 引用核查已完成 → 编译最终答案
    3. 已有分析结果：
       - 检索到原著资料 → verifier（引用核查）
       - 无资料可对照 → 直接作为最终回答
    4. 已检索但未分析 → analyzer（防死循环）
    5. 问候/闲聊 → 直接回复
    6. 其余 → retriever（先检索，analyzer 会基于检索结果回答）
    """
    config = get_scene_config()
    query = state["query"]
    ...
    # 已有分析结果：
    # - 检索到原著资料 → 先做引用核查（verifier 标注出处与可信度），再编译最终答案
    # - 无资料可对照（闲聊/无检索结果）→ 分析已以角色口吻流式生成，直接作为最终回答
    if has_analysis and not has_verification:
        if has_docs and state.get("enable_verification", True):
            print("[Supervisor] 分析完成且有原著资料，路由到 verifier 做引用核查")
            return {
                "next_agent": "verifier",
                "route_history": route_history + ["supervisor"],
            }
        if has_docs and not state.get("enable_verification", True):
            print("[Supervisor] 沉浸模式已关闭引用核查，分析结果直接作为最终回答")
    ...
    # ---- 问候/闲聊直接回复，其余一律先检索（analyzer 会基于检索结果回答）----
    # persona 场景几乎总是需要"检索 + 角色口吻回答"，跳过 LLM 路由省一次调用
    greeting = len(query) <= 20 and any(
        kw in query
        for kw in ["你好", "您好", "嗨", "哈喽", "hello", "hi", "在吗", "你是谁", "谢谢", "再见", "拜拜", "辛苦了", "早上好", "晚上好"]
    )
    if greeting:
        print("[Supervisor] 名人对话：问候语，直接回复")
        final_answer = await _generate_direct_response(
            query, history, character_role_prompt, stream_callback,
            session_id=state.get("session_id"),
        )
...
def _compile_final_answer(state: AgentState) -> str:
    ...
    if verification:
        # 提取核查摘要与引用出处（拼接到回答末尾，突出"基于原著"的可信度）
